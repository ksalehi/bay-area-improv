...
import { fetchTheaters, theaterSlug } from "@/lib/theaters";
...
export default async function ClassesPage() {
  const [groups, theaters] = await Promise.all([fetchClasses(), fetchTheaters()]);
  const theatersByName = new Map(theaters.map((t) => [t.name.trim().toLowerCase(), t]));
    ...
    <main className="mx-auto max-w-4xl px-6 py-12">
      ...
      <div className="divide-y divide-[#e8e3de]">
        {groups.map(({ theater, classes }) => {
          const matchedTheater = theatersByName.get(theater.trim().toLowerCase());
          return (
            <section key={theater} className="py-10 first:pt-0 last:pb-0">
              <div className="mb-4">
                <h2 className="text-lg font-semibold text-[#1c1917] tracking-tight">
                  {matchedTheater ? (
                    <Link
                      href={`/theaters#${theaterSlug(matchedTheater.name)}`}
                ...
                {matchedTheater?.city && (
                  <p className="text-sm text-[#9c948e] mt-0.5">{matchedTheater.city}, CA</p>
                )}
              </div>
